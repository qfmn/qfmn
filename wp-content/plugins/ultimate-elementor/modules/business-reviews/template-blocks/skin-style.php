	...
	public function get_google_api( $settings ) {
		...
		if ( '' !== $integration_options['google_places_api'] ) {
			...
			if ( 'yes' === self::$google_api_status ) {
				...
				echo __( '<span class="uael-reviews-notice-message"><span class="uael-reviews-error-message">Google Error Message:</span> Incorrect Google API key! Please set up the API key from UAE settings.</span>', 'uael' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				return false;
			}
		} elseif ( $is_editor ) {
			echo __( '<span class="uael-reviews-notice-message"><span class="uael-reviews-error-message">Google Error Message:</span> Please configure the Google API key to display the reviews.</span>', 'uael' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			return false;
		} else {
			return false;
		}
		...
		if ( false === $result ) {
			...
			if ( is_wp_error( $result ) ) {
				...
				if ( $is_editor ) {
					...
					echo sprintf( __( '<span class="uael-reviews-notice-message">Something went wrong while fetching Google reviews: %1$s</span>', 'uael' ), esc_attr( $error_message ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				}
				return;
			}
	...
	public function get_yelp_api( $settings ) {
		...
		if ( '' !== $business_id ) {
			...
			if ( '' !== $integration_options['yelp_api'] ) {
				...
				echo __( '<span class="uael-reviews-notice-message"><span class="uael-reviews-error-message">Yelp Error Message:</span> Please set the Yelp API key to display the reviews.</span>', 'uael' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				return false;
			} else {
				return false;
			}
			...
			if ( is_wp_error( $result ) ) {
				...
				if ( $is_editor ) {
					/* translators: %1$s doc link */
					echo sprintf( __( '<span class="uael-reviews-notice-message">Something went wrong: %1$s</span>', 'uael' ), esc_attr( $error_message ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				}
				delete_transient( $transient_name );
				return;
			}
			...
			if ( 200 !== $response_code && ! empty( $response_message ) ) {
				if ( $is_editor ) {
					...
					if ( 'VALIDATION_ERROR' === $error_code ) {
						echo __( '<span class="uael-reviews-notice-message"><span class="uael-reviews-error-message">Yelp Error Message:</span> Incorrect Yelp API key. Please set up the API key from UAE settings.</span>', 'uael' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					} elseif ( 'BUSINESS_NOT_FOUND' === $error_code ) {
						echo __( '<span class="uael-reviews-notice-message"><span class="uael-reviews-error-message">Yelp Error Message:</span> Incorrect Business ID.</span>', 'uael' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					}
				}
				delete_transient( $transient_name );
				return;
				...
				if ( $is_editor ) {
					/* translators: %1$s doc link */
					echo sprintf( __( '<span class="uael-reviews-notice-message"> %1$s Unknown error occurred.</span>', 'uael' ), esc_attr( $response_code ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				}
				delete_transient( $transient_name );
				return;
			}
