...
class UAEL_Core_Plugin {
	...
	private function setup_actions_filters() {
		...
		if ( UAEL_Helper::is_widget_active( 'Cross_Domain' ) ) {
			add_action( 'elementor/editor/before_enqueue_scripts', array( $this, 'enqueue_copy_paste_scripts' ) );
	...
	public function enqueue_copy_paste_scripts() {
		...
		wp_localize_script(
			...
			array(
				...
				'cross_domain_cdn'  => apply_filters( 'uael_cross_domain_cdn', 'https://brainstormforce.github.io/uae-cdcp/index.html' ),
			)
		);
